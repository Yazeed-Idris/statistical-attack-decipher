...
mixed_trigrams = window(cipher_text, 3)
trigrams_list = {}
for word in mixed_trigrams:
    if len(word.replace(' ', '')) == 3:
        trigrams_list[word] += 1
...
for word in my_dict:
    if len(word) > 3:
        end_with_ing[word[-3:]] = 0
for word in my_dict:
    if len(word) > 3:
        end_with_ing[word[-3:]] += 1
print(end_with_ing)
...
for word, count in my_dict.items():
    if len(word) == 2:
        diagrams_list[word] = count
# Finding the highest 2 frequencies in the trigrams and removing them from the list
max_diagram1 = max_in_dict(diagrams_list)
diagrams_list.pop(max_diagram1)
# max_diagram2 = max_in_dict(diagrams_list)
# diagrams_list.pop(max_diagram2)
...
# for i in range(len(max_diagram1)):
#     cipher_text = cipher_text.replace(max_diagram1[i], common_diagram[i])

# Calculate character frequencies
charFrequincies = {}
for i in range(len(alphabet)):
    charFrequincies[alphabet[i]] = cipher_text.count(alphabet[i])
print(charFrequincies)
